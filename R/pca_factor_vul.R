...
pca_factor_vul <- function(vul.data){
  
 
  data.set.wide <- vul.data %>% 
  ...
  ggplot2::ggsave(here::here("outputs","figures",paste0("variables_pca_vulnerability.png")), variable.plot, device="png", width = 8, height = 5)
  ...
  scaled.pca <- scale(this.pca$pc_val, center = mins, scale = maxs - mins) %>% 
    ...
    dplyr::rename(norm_pca_vul = V1) 
  
  print(scaled.pca)
  
  readr::write_csv(scaled.pca,here::here("outputs","analysis","vul_factor_scores.csv"))
