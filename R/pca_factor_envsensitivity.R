...
pca_factor_envsensitivity <- function(eachscenario, scaledenvsens, scenario.list){
  ...
  data.set <- scaledenvsens %>%
    dplyr::filter(scenario == this.scenario)
  
  data.set.wide <- data.set %>% 
    tidyr::pivot_wider(names_from = var_name, values_from=value) %>% 
    dplyr::select(-c(localidad, municipio, entidad, CVE_LOC, cooperativa))
  ...
  scaled.pca <- scale(this.pca$pc_val, center = mins, scale = maxs - mins) %>% 
    ...
    dplyr::rename(norm_pca_envsens = V1) %>% 
    dplyr::mutate(scenario = this.scenario)
  
  print(scaled.pca)
  
  readr::write_csv(scaled.pca,here::here("outputs","analysis",paste0("envsens_factor_scores_",eachscenario,".csv")))
