...
pca_factor_sensitivity <- function(eachscenario, sensitivitysp, scenario.list){
  
  print(eachscenario)
  this.scenario <- scenario.list[eachscenario]
  
  print(this.scenario)
  datasetname <- paste0("sensitivity_", this.scenario)
  
  print(datasetname)
  
  data.set.wide <- sensitivitysp %>%
    tidyr::pivot_wider(names_from = var_name, values_from=value) %>% 
    dplyr::select(-c(nombre_comun, localidad, municipio, entidad, CVE_LOC, species))
  
  print(head(data.set.wide))
  
  data.set <- data.set.wide %>%
    dplyr::filter(scenario == this.scenario) 
  
  data.set.frame <- data.set %>%
    dplyr::select(-scenario, -nom_coop) %>% 
    as.data.frame
  ...
  ggplot2::ggsave(here::here("outputs","figures",paste0("variables_pca_",this.scenario,".png")), variable.plot, device="png", width = 8, height = 5)
  ...
  scaled.pca <- scale(this.pca$pc_val, center = mins, scale = maxs - mins) %>% 
    ...
    dplyr::rename(norm_pca_spsens = V1) %>% 
    dplyr::mutate(scenario = this.scenario)
  
  print(scaled.pca)
  
  readr::write_csv(scaled.pca,here::here("outputs","analysis",paste0("spsens_factor_scores_",eachscenario,".csv")))
